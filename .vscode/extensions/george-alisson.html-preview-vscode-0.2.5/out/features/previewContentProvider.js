...
class HTMLContentProvider {
    constructor(context, cspArbiter, logger) {
        ...
        this.logger = logger;
        this.TAG_RegEx = /^\s*?\<(p|h[1-6]|img|code|pre|blockquote|li|dt|dd|td|th)((\s+.*?)(class="(.*?)")(.*?\>)|\>|\>|\/\>|\s+.*?\>)/;
    }
    provideTextDocumentContent(htmlDocument, previewConfigurations, initialLine = undefined, state) {
        ...
        const parsedDoc = htmlDocument.getText().split("\n").map((l, i) => l.replace(this.TAG_RegEx, (match, p1, p2, p3, p4, p5, p6, offset) => match.replace(match, typeof p5 !== "string" ?
            `<${p1} class="code-line" data-line="${i}" ${p2}` :
            `<${p1} ${p3} class="${p5} code-line" data-line="${i}" ${p6}`))).join("\n");
